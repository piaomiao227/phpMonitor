...
class Email {
    ...
    static public function init($config = array()) {
        ...
        self::$config['SMTP_USERNAME'] = isset($config['SMTP_USERNAME']) ? $config['SMTP_USERNAME'] : '1000@qq.com'; //smtp服务器帐号，如：你的qq邮箱
        ...
        self::$config['SMTP_FROM_TO'] = isset($config['SMTP_FROM_TO']) ? $config['SMTP_FROM_TO'] : '1000@qq.com'; //发件人邮件地址
        self::$config['SMTP_FROM_NAME'] = isset($config['SMTP_FROM_NAME']) ? $config['SMTP_FROM_NAME'] : 'phpMonitor'; //发件人姓名
        self::$config['SMTP_DEBUG'] = isset($config['SMTP_DEBUG']) ? $config['SMTP_DEBUG'] : true; //是否显示调试信息	
    }
    ...
    static public function send($mail_to, $mail_subject, $mail_body, $mail_attach = NULL) {
        ...
        if (!isset(self::$config) || empty(self::$config)) {
            $mailConfig = C('mailConfig');
            self::init($mailConfig);
        }
